...
`INV-2026-0007`. Exact prefix is a config value, confirmed with you before Phase 1 build.

## 3.9 Open questions for you before Phase 1 build

1. **Numbering prefixes** — confirm `CTS-` / `QT-` / `INV-` or your preferred scheme, and
   whether numbering resets each calendar year (as modeled) or runs continuously.
2. **VAT rate** — confirm 15% as the default (South African standard rate) and whether any
   jobs are VAT-exempt (e.g. export/zero-rated).
3. **Multiple technicians per job** — confirm single assignee is sufficient for MVP, or if
   crew assignment is needed from day one.
4. **Customer approval mechanism** — is an emailed link with an "Approve Quote" button enough
   for MVP, or is a signature/OTP-verified approval required from the start?
